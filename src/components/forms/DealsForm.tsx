...
const DealsForm = ({
  ...
  const myDeal: Deal | undefined = data.deals.find(
    (deal: any) => deal.name === formData.name
  );
  ...
  return (
    ...
    >
      ...
      {(mode === 'add' ||
        (mode === 'edit' &&
          (userRole === 'ADMIN' || myDeal?.created_by?.id === userId))) && (
